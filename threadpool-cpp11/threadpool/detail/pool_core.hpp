...
public: // Type definitions
    ...
    //cjh    static_assert(std::function_traits<task_type()>::arity == 0, "");
    //
    //    //
    static_assert(std::is_void<typename std::result_of<task_type()>::type >::value, "The task function's result type is required to be void.");
...
private: // The following members are accessed only by _one_ thread at the same time:
    ...
    std::unique_ptr<size_policy_type> m_size_policy; // is never null

    bool  m_terminate_all_workers;								// Indicates if termination of all workers was triggered.
    std::vector<std::shared_ptr<worker_type> > m_terminated_workers; // List of workers which are terminated but not fully destructed.
...
public:
    ...
    {
        locking_ptr<pool_type, std::recursive_mutex> lockedThis(*this, m_monitor);
    ...
    {
        locking_ptr<const pool_type, std::recursive_mutex> lockedThis(*this, m_monitor);
    ...
    void wait(size_t const task_threshold = 0) const volatile
    {
        ...
        std::unique_lock<std::recursive_mutex> lock(self->m_monitor);//cjh
        //cjh     recursive_mutex::scoped_lock lock(self->m_monitor);
        if (0 == task_threshold)
        {
    ...
    /*! The current thread of execution is blocked until the timestamp is met
                ...
                */
    //cjh    bool wait(xtime const & timestamp, size_t const task_threshold = 0) const volatile
    //    {
    //      const pool_type* self = const_cast<const pool_type*>(this);
...
private:
    ...
    bool resize(size_t const worker_count) volatile
    {
        ...
        { // increase worker count
            while (m_worker_count < m_target_worker_count)
            {
                ...
                catch (...)//cjh thread_resource_error)
                {
                    return false;
                }
            }
    ...
    bool execute_task() volatile
    {
        ...
        { // fetch task
            ...
            std::unique_lock<std::recursive_mutex> lock(lockedThis->m_monitor);//cjh
            //cjh        recursive_mutex::scoped_lock lock(lockedThis->m_monitor);
