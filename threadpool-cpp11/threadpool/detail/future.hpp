	...
	namespace threadpool
	{
		namespace detail
		{
			...
			private:
				...
				mutable std::condition_variable m_condition_ready;
			...
			public:
				...
				void wait() const volatile
				{
					...
					//cjh mutex::scoped_lock lock(self->m_monitor);

					std::unique_lock<std::mutex> lock(self->m_monitor); //cjh
				...
				{
					locking_ptr<future_type, std::mutex> lockedThis(*this, m_monitor);
			...
			public:
				...
				typedef typename std::result_of<function_type()>::type future_result_type; //!< Indicates the future's result type.
				typedef Future<future_result_type> future_type;   //!< Indicates the future's type.

				// The task is required to be a nullary function.
			  //cjh  BOOST_STATIC_ASSERT(function_traits<function_type()>::arity == 0);

				static_assert(!std::is_void<future_result_type>::value, "The task function's result type is required not to be void");

			private:
				function_type             m_function;
				std::shared_ptr<future_type>   m_future;

			public:
				future_impl_task_func(function_type const & function, std::shared_ptr<future_type> const & future)
					: m_function(function)
					, m_future(future)
				{
				}
