...
class thread_pool
{
    ...
    std::shared_ptr<pool_core_type>          m_core; // pimpl idiom
    std::shared_ptr<void>                    m_shutdown_controller; // If the last pool holding a pointer to the core is deleted the controller shuts the pool down.
...
public:
    ...
    */
    //cjh    bool wait(xtime const & timestamp, size_t task_threshold = 0) const
    //    {
